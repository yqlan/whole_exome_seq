...
#PBS -N WGC034810U
#PBS -q middleq
#PBS -e WGC034810U_script_err
#PBS -o WGC034810U_script_log
#PBS -l mem=60gb,walltime=100:00:00,nodes=1:ppn=15
#HSCHED -s hschedd

#############################################################################################################################
cd /leofs/mishl_group/liangl/rare_dis/rg
name=WGC034810U
...
/software/biosoft/software/bwa-0.7.10/bwa sampe -r "@RG\tID:WGC810\tLB:WGC810\tPL:illumina\tSM:WGC810" ${ref}.fa ${name}_R1.sai ${name}_R2.sai ${R1}.fastq ${R2}.fastq > ${name}.sam 2>>${name}_err
